...
//	m_Book.SaveCopyAs((COleVariant)strSaveName);
	m_Book.SaveAs(COleVariant(strSaveName), COleVariant((short)-4143),  //xlnormal=-4143  
                        COleVariant(_T("")),COleVariant(_T("")),covFalse,covTrue,
...
void CXLControl::ExcelExecuteFile(CString strFileName)
{
	...
	ShellExecute(hWnd,_T("open"),strFileName,NULL,NULL,SW_SHOWNORMAL);
}
	...
	if (hProcessSnap != INVALID_HANDLE_VALUE) 
	{
		...
		{ 
			...
			{ 
				...
				wchar_t * Temp = /*_wcsupr*/(pe32.szExeFile);
				if (wcsstr(Temp, _T("EXCEL.EXE")) != 0 )
